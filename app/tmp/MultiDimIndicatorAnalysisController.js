...
export default function MultiDimIndicatorAnalysisController($scope, $state, $filter, HttpRequestService) {
    ...
    //     $scope.indicatorSelectedOptions = $scope.indicatorModel;
    // });
            ...
            HttpRequestService.get(url, params, response => {
                ...
                if (response.entities) {
                    response.entities.map(item => {
                        $scope.userAccountModel.push(item.userAccount);
                        $scope.userAccountOptions.push(item.userAccount);
                    });
                }
            })
        },
    ...
    const selectedTemplate = template => {
        isRecover = 2;
        $scope.userType = template.userType;
        $scope.dimensionsModel = template.dimensionsModel;
        getData.getIndicatorlist();
        ...
        $scope.subAppModel = template.subApp;
        $scope.indicatorModel = template.indicatorModel;
        $scope.indicatorSelectedOptions = template.indicatorModel;
    ...
    const getDataForChart = indicator => {
        ...
        HttpRequestService.get(url, requestParams, response => {
            if (response.entities) {
                response.entities.map(item => {
                    switch (requestParams.dimension) {
                        case 'device': {
                            dimensionValue += (item.deviceName + ',');
                            break;
                        }
                        case 'app': {
            ...
            if (requestParamsR.dimension != 'app' && dimensionValue.length != 0) {
